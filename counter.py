...
lowTariff = 0.2208 #per kWh price for low tariff
highTariff = 0.2848 #per kWh price for high tariff
...
class electricityCounter():
	...
	def countPulse(self,pin):
		...
		if now > self.lastLog + self.logInterval:
			...
			logString += f'chargerLow:,{self.pulses * int(not counterStatus["highTariff"]) * int(not counterStatus["ssrOn"])}\n'
